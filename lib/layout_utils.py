...
class CocoLocationMap(object):
    def __init__(self, config):
        ...
        self.cols, self.col_step = \
            np.linspace(self.margin, 1.0 - self.margin, num=config.grid_size[0],
                        endpoint=True, retstep=True, dtype=np.float64)
    ...
    def coords2indices(self, coords):
        ...
        grids = (grids + 0.5).astype(np.uint8)
...
class CocoTransformationMap(object):
    ...
    def coords2indices(self, coords):
        scale_indices = ((coords[:, 0] - self.margin) / self.scale_step + 0.5).astype(np.uint8)
...
def log_coco_scores(infos, path):
    ...
    log_info['unigram_coord'] = [np.mean(unigram_coord), np.std(unigram_coord), np.amin(unigram_coord),
                                 np.amax(unigram_coord)]
    bigram_coord = infos.bigram_coord()
    log_info['bigram_coord'] = [np.mean(bigram_coord), np.std(bigram_coord), np.amin(bigram_coord),
                                np.amax(bigram_coord)]
